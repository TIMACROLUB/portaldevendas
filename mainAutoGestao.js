...
window.addEventListener("DOMContentLoaded", function(event) {
  ...
  const renderOrder = ({NUMEROPEDIDO, NOMECLIENTE,ETAPAATUAL}) => {
    ...
    const colOrderData = html.create('div', 'col');
      const h5NumeroPedido = html.create('h5','',NUMEROPEDIDO);
      const h5NomeCliente = html.create('h5','',NOMECLIENTE);
    const colStatus = html.create('div','col d-flex align-items-center justify-content-end text-right');
      const h3Status = html.create('div', '', ETAPAATUAL);
  ...
  const botaoConsultarPedido = html.get('#btn-consulta-pedido');
  console.log(botaoConsultarPedido);
  botaoConsultarPedido.addEventListener('click', e=> {
    ...
    (pedido)=>{
      ...
      const content = renderOrder({...pedido[0]});
